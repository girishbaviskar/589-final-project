...
def test_back_prop_titanic_dataset():
    pddf = read_data_file('../../datasets/titanic.csv')
    X, y = splitArgumentsAndLabel(pddf)
    one_hot_encoded_data_X = pd.get_dummies(X, columns=['Pclass', 'Sex'], dtype=int)
    X = one_hot_encoded_data_X.to_numpy()
...
if __name__ == "__main__":
    ...
    reg_lambda = 0.01
    neurons_per_layer = [9, 8, 8, 2]  # bias terms are not included in this architecture
    iterations = 500
    mini_batch_size = 400
    alpha = .5


    X, y, folds_indexes_list  = test_back_prop_titanic_dataset()
    #drop loan id column
    X = X[:, 1:]
    ...
    for train, test in folds_indexes_list:
        ...
        for b in range(iterations):
            for i in range(0, m, mini_batch_size):
                ...
                J += J_b
                list_of_J_for_each_batch.append(J_b/mini_batch_size)
    ...
    plotGraph(x_axis_val, mean_list, 'Error(J) vs number batch for titanic dataset', "N_batches", "J", 0)
